...
// ══ CONFIG ══════════════════════════════════════════════════════
// En production Vite → import.meta.env.VITE_SUPABASE_URL
// Dans l'artifact sandbox → ces variables n'existent pas, on tombe en mode démo.
// NE PAS modifier cette logique : elle est correcte pour les deux contextes.
// ── CONFIG ──────────────────────────────────────────────────────
// En production Vite : ces valeurs sont injectées depuis .env.local
//   VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY
// Dans l'artifact sandbox : import.meta n'est pas disponible → mode démo automatique.
// Ne PAS modifier cette logique : elle est intentionnelle pour les deux contextes.
const SB_URL = (() => {
  try { return __VITE_SUPABASE_URL__ || "DEMO"; } catch { return "DEMO"; }
})();
const SB_KEY = (() => {
  try { return __VITE_SUPABASE_ANON_KEY__ || "DEMO"; } catch { return "DEMO"; }
})();
const SB_READY = SB_URL !== "DEMO" && SB_KEY !== "DEMO"
              && SB_URL.startsWith("https://") && SB_KEY.length > 20;

// IA : TOUJOURS via /api/analyse-reco en production (clé Anthropic côté serveur).
// Dans l'artifact : fallback appel direct avec erreur explicite si clé absente.
...
const sbErrors = []; // log des erreurs Supabase pour le diagnostic
...
const sb = {
  ...
  async signIn(email, pwd) {
    ...
    // Persister token — survie au refresh
...
// ══ DÉTECTION DOUBLON v2 ════════════════════════════════════════
// Règle 1 — competitor_id renseigné :
//   doublon = même (week_id + competitor_id + capacity + collected_at + source)
// Règle 2 — competitor_id null :
//   doublon = même (week_id + property_name + source + capacity + collected_at)
...
// Enrichit les relevés avec les données du concurrent
...
async function getCompetitorRates({ weekId, capacity, showExamples = false }, allCompetitors) {
  let raw = [];
  if (SB_READY) {
    // Récupère les données du concurrent lié via select imbriqué
...
async function saveCompetitorRate(rate, allCompetitors) {
  // Historisation garantie : on INSERT toujours, jamais d'UPDATE.
  // Doublon v2 : deux règles selon que competitor_id est présent ou non.
  const clean = stripUserId(rate); // user_id géré par Supabase DEFAULT auth.uid()

  if (SB_READY) {
    // Vérification doublon côté Supabase avant insert
...
// ══ IMPORT CSV — matching competitor_id v2 ════════════════════════
// Ordre de matching :
//  1. competitor_id exact (colonne CSV)
//  2. property_name exact sur competitors.name
//  3. null (pas de matching sur source seul — Booking peut désigner plusieurs types)
function matchCompetitor(row, competitors) {
  if (row.competitor_id) {
    const found = competitors.find(c => c.id === row.competitor_id);
    if (found) return found.id;
  }
  if (row.property_name) {
    const found = competitors.find(c => c.name === row.property_name);
    if (found) return found.id;
  }
  return null; // pas de matching automatique sur source
}
function median(arr) {
...
export default function App() {
  ...
  // Copier-coller avec formulaire de validation éditable
  ...
  const [pasteEdit, setPasteEdit] = useState(null); // formulaire éditable après extraction
  ...
  // ── Auth ──────────────────────────────────────────────────────
  useEffect(() => {
    // Restaurer session au chargement (survie au refresh)
  ...
  async function handleImportCsv() {
    ...
    const rows = lines.slice(1).map((line, li) => {
      ...
      // Trouver competitor_id via property_name ou source
  ...
  // ── Analyse IA : TOUJOURS via /api/analyse-reco en prod ─────────
  // Dans l'artifact sandbox : fallback direct Anthropic (affiche erreur explicite si manque clé)
  async function runIA() {
    ...
    try {
      // Tentative /api/analyse-reco (production Vercel)
      let res = await fetch(IA_ENDPOINT, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      if (res.status === 404) {
        // Fallback artifact : appel direct (pas de clé → erreur explicite)
        ...
        if (!res.ok) {
          ...
          throw new Error(
            ...
            "\n→ En production : déployer api/analyse-reco.js sur Vercel avec ANTHROPIC_API_KEY côté serveur."
          );
        }
...
4. ACTION : une action immédiate et concrète`;
          ...
          {tab==="detail" && !ratesLoading && (<>
            ...
            {allMax>allMin&&ourPrice>0&&(
              <div style={{ ...cd(11), padding:"10px 12px" }}>
                <p style={{ margin:"0 0 4px", fontSize:10, color:C.gray }}>Position sur le marché (vraie médiane statistique)</p>
          ...
          {tab==="reco" && (<>
            <div style={{ ...cd(13), padding:"12px" }}>
              ...
              <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                ...
                {reco.excludedCount>0&&<Badge label={`${reco.excludedCount} exclus (score<${settings.minScore})`} color={C.gray} bg={C.grayL}/>}
            ...
            {iaError&&<div style={{ ...cd(10), padding:"9px 12px", background:C.redL, marginBottom:6 }}><p style={{ margin:0, fontSize:11, color:C.red }}>✗ {iaError}</p><p style={{ margin:"4px 0 0", fontSize:10, color:C.red }}>En production : route /api/analyse-reco (Vercel serverless) avec clé Anthropic côté serveur.</p></div>}
  ...
  // ══ COLLECTE ═════════════════════════════════════════════════
  const Collect = () => {
    // ── Copier-coller avec formulaire éditable ──
        ...
        <div style={cnt}>
          ...
          {!pasteEdit ? (<>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:6 }}>
              ...
              <div><p style={sml}>Concurrent concerné</p><select value={pasteCompId} onChange={e=>setPComp(e.target.value)} style={inp()}>{competitors.map(c=><option key={c.id} value={c.id}>{c.name.slice(0,22)}</option>)}</select></div>
            </div>
            <p style={sml}>Texte collé depuis la page</p>
            <textarea value={pasteRaw} onChange={e=>setPasteRaw(e.target.value)} placeholder={"Colle ici le texte de la page Booking, Airbnb ou Abritel...\n\nEx :\nLes Chalets du Verdon\n4,5/5 · 128 avis\nGenius -10%\n580 € / semaine\nfrais de ménage : 45 €"} style={{ width:"100%", minHeight:110, padding:"9px", fontSize:11, border:`1px solid ${C.grayM}`, borderRadius:10, background:C.grayL, color:C.text, resize:"vertical", boxSizing:"border-box", marginBottom:6 }}/>
            <button style={btn(!pasteRaw.trim(),C.purple)} onClick={handleParse} disabled={!pasteRaw.trim()}>🔍 Analyser le texte →</button>
          </>) : (<>
            {/* Formulaire de validation éditable */}
            <div style={{ ...cd(11), padding:"10px 13px", background:pasteEdit.warning?C.orangeL:C.greenL, marginBottom:8 }}>
              ...
              {pasteEdit.allPrices?.length>0&&<p style={{ margin:0, fontSize:10, color:C.textS }}>Prix détectés dans le texte : {pasteEdit.allPrices.map(p=>`${p}€`).join(" · ")}</p>}
            </div>
            <p style={{ margin:"0 0 8px", fontSize:11, fontWeight:600, color:C.text }}>Formulaire de validation éditable</p>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:6 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:6 }}>
              <div><p style={sml}>Capacité</p><select value={pasteEdit.capacity} onChange={e=>setPasteEdit({...pasteEdit,capacity:parseInt(e.target.value)})} style={inp()}>{[2,4,6,8].map(n=><option key={n} value={n}>{n} pers.</option>)}</select></div>
              <div><p style={sml}>Concurrent</p><select value={pasteEdit.competitorId} onChange={e=>setPasteEdit({...pasteEdit,competitorId:e.target.value})} style={inp()}>{competitors.map(c=><option key={c.id} value={c.id}>{c.name.slice(0,20)}</option>)}</select></div>
            </div>
            <SaveFeedback/>
            <button style={btn(pasteSaving||!pasteEdit.priceWeek,C.blue)} onClick={handleSavePaste} disabled={pasteSaving||!pasteEdit.priceWeek}>{pasteSaving?"Enregistrement…":`Valider et enregistrer dans ${SB_READY?"Supabase":"mémoire locale"} ✓`}</button>
            <button style={{ ...btn(false,C.grayL,C.textS), marginTop:-2 }} onClick={()=>{setPasteEdit(null);}}>← Recommencer l'extraction</button>
            <p style={{ fontSize:9, color:C.gray, textAlign:"center" }}>Statut : <strong>à vérifier</strong> · méthode : copier-coller · les relevés passés ne sont jamais écrasés</p>
          </>)}
        </div><BNav/>
      </div>
    );
    ...
    return (
      <div><SBar title="Saisie rapide"/>
        <div style={cnt}>
          ...
          <button style={btn(!form.priceWeek)} onClick={handleSaveForm} disabled={!form.priceWeek}>Enregistrer — méthode : saisie manuelle ✓</button>
          <p style={{ fontSize:9, color:C.gray, textAlign:"center" }}>Les anciens relevés ne sont jamais écrasés · chaque date = un enregistrement distinct</p>
        </div><BNav/>
      </div>
    );
  };
    ...
    <div><SBar title="Import CSV"/>
      <div style={cnt}>
        <div style={{ ...cd(11), padding:"10px 13px", background:C.bluePale, marginBottom:8 }}>
          ...
          <p style={{ margin:0, fontSize:9, color:C.blueL, fontFamily:"monospace", lineHeight:1.6 }}>week_start · source · property_name · property_type · capacity · price_week · price_night · original_price · promo_label · promo_percent · cleaning_fee · url · collected_at · reliability_status</p>
        </div>
        {csvResult&&(
          <div style={{ ...cd(10), padding:"10px 12px", background:csvResult.errors.length===0?C.greenL:C.goldL, marginBottom:8 }}>
            ...
            <p style={{ margin:"0 0 1px", fontSize:11, color:C.gray }}>⊝ Ignorées (champs manquants) : {csvResult.skipped}</p>
            {csvResult.errors.map((e,i)=><p key={i} style={{ margin:0, fontSize:10, color:C.red }}>✗ {e}</p>)}
          </div>
        )}
        <p style={sml}>Coller le CSV</p>
        <textarea value={csvText} onChange={e=>setCsvText(e.target.value)}
          placeholder={"week_start;source;property_name;property_type;capacity;price_week;price_night;original_price;promo_label;promo_percent;cleaning_fee;url;collected_at;reliability_status\n2026-08-01;Airbnb;Appt 6p La Foux;particulier;6;680;97;;Remise 7 nuits -5%;5;0;https://airbnb.fr;2026-05-20;réel"}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:6 }}>
          ...
          <button onClick={()=>{
            // Modèle CSV v2 avec competitor_id
            const template = [
              ...
              "2026-08-01;Booking;cv;Les Chalets du Verdon;résidence;6;620;89;680;Genius -10%;10;0;https://booking.com/chalets-verdon;"+new Date().toISOString().slice(0,10)+";réel",
              "2026-08-01;Airbnb;;Appt particulier La Foux;particulier;6;680;97;;;0;45;https://airbnb.fr/rooms/123;"+new Date().toISOString().slice(0,10)+";saisi manuellement",
            ].join("\n");
            const b=new Blob([template],{type:"text/csv;charset=utf-8"});
            const u=URL.createObjectURL(b); const a=document.createElement("a");
            a.href=u; a.download="template_benchmark_v2.csv"; a.click();
          }} style={{ ...btn(false,C.grayL,C.blueL), margin:0, border:`1px solid ${C.blueL}` }}>⬇ Modèle v2</button>
        </div>
        <button style={btn(csvLoading||!csvText.trim())} onClick={handleImportCsv} disabled={csvLoading||!csvText.trim()}>{csvLoading?"Import en cours…":`Importer dans ${SB_READY?"Supabase":"mémoire locale"} →`}</button>
  ...
  const Diagnostic = () => {
    ...
    const envDetected = false; // artifact : toujours démo
    const iaMode     = "→ /api/analyse-reco (Vercel) avec fallback artifact";
    ...
    return (
      <div><SBar title="Diagnostic"/>
        <div style={cnt}>
          ...
          <div style={cd()}>
            {[
              { l:"VITE_SUPABASE_URL détecté", v:envDetected?"Oui":"Non (mode démo)",     c:envDetected?C.green:C.red },
              { l:"Mode stockage",             v:SB_READY?"Supabase":"Local (localStorage)", c:SB_READY?C.green:C.gold },
              { l:"IA endpoint",               v:iaMode,                                     c:C.blue },
              { l:"RLS Supabase",              v:SB_READY?"Supposé actif (via SQL)":"N/A", c:SB_READY?C.green:C.gray },
              { l:"Session",                   v:user?.email||"Non connecté",               c:user?C.green:C.red },
              { l:"Token persistant",          v:sessionStorage.getItem("sb_token")?"Oui (sessionStorage)":"Non", c:sessionStorage.getItem("sb_token")?C.green:C.gray },
            ].map((r,i,arr) => (
          ...
          <div style={cd()}>
            {[
              ...
              { l:"Stockés localement (toutes sem.)", v:totalLocal,         c:totalLocal>0?C.blue:C.gray },
            ].map((r,i,arr) => (
              <div key={r.l} style={rw(i===arr.length-1)}>
                <span style={{ fontSize:11, color:C.text }}>{r.l}</span>
          ...
          {lastImport && (<>
            <p style={sml}>Dernier import</p>
            <div style={{ ...cd(11), padding:"10px 13px" }}>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                <Badge label={`✓ ${lastImport.rows_imported} importées`}   color={C.green}  bg={C.greenL}/>
              ...
              {lastImportStats && (
                <div style={{ marginTop:6, display:"flex", gap:6, flexWrap:"wrap" }}>
                  <Badge label={`Sans competitor_id : ${lastImportStats.noComp}`} color={lastImportStats.noComp>0?C.orange:C.green} bg={lastImportStats.noComp>0?C.orangeL:C.greenL}/>
          ...
          {sbErrors.length > 0 && (<>
            <p style={sml}>Erreurs Supabase ({sbErrors.length})</p>
            <div style={cd()}>
              {sbErrors.slice(-3).reverse().map((e,i,arr) => (
                <div key={i} style={rw(i===arr.length-1)}>
                  <div>
                    <p style={{ margin:0, fontSize:11, fontWeight:500, color:C.red }}>{e.path}</p>
                    <p style={{ margin:0, fontSize:10, color:C.textS }}>{e.ts?.slice(11,19)} — {e.msg?.slice(0,60)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={cd()}>
            {[
              { l:"VITE_SUPABASE_URL configuré",       ok:envDetected },
              { l:"Mode Supabase actif",                ok:SB_READY },
              { l:"Session persistante (sessionStorage)", ok:!!sessionStorage.getItem("sb_token") },
              ...
              <div key={c.l} style={rw(i===arr.length-1)}>
                <div>
                  <span style={{ fontSize:11, color:C.text }}>{c.l}</span>
                  {c.note && <p style={{ margin:"1px 0 0", fontSize:9, color:C.gray }}>{c.note}</p>}
                </div>
