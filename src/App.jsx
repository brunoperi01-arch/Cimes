...
export default function App() {
  ...
  const Weeks=()=>{
    const ws=STATIC_WEEKS.filter(w=>w.year===yr); const grouped={};
    ws.forEach(w=>{ if(!grouped[w.month_label]) grouped[w.month_label]=[]; grouped[w.month_label].push(w); });
    return (
      <div><SBar title={`Semaines ${yr}`}/>
        <div style={{ padding:"4px 14px 0" }}><div style={{ display:"flex", gap:5, marginBottom:6 }}>{[2026,2027].map(y=><button key={y} onClick={()=>setYr(y)} style={{ padding:"5px 10px", fontSize:11, fontWeight:yr===y?700:400, background:yr===y?C.blue:C.white, color:yr===y?C.white:C.text, border:"none", borderRadius:16, cursor:"pointer" }}>{y}</button>)}</div></div>
        <div style={{ ...cnt, paddingTop:2 }}>
          {Object.entries(grouped).map(([ml,wks])=>(
            <div key={ml}><p style={sml}>{ml}</p>
              <div style={cd()}>
                {wks.map((w,i)=>{ const op=OUR_TARIFS[cap]?.[w.season_type]||0; return (
                  <div key={w.id} onClick={()=>{ setSWId(w.id); setTab("detail"); setIaText(null); setScreen("week"); }} style={{ ...rw(i===wks.length-1), cursor:"pointer" }}>
                    ...
                    <div style={{ textAlign:"right" }}>{op>0&&<><p style={{ margin:0, fontSize:12, fontWeight:600, color:C.blue }}>{fmt(Math.round(op/7))}€/n</p><p style={{ margin:0, fontSize:9, color:C.gray }}>{fmt(op)}€/sem</p></>}</div>
                  </div>
                );})}
              </div>
            </div>
          ...
          {tab==="table"&&!ratesLoading&&(<>
            ...
            <div style={{ marginTop:8 }}>
              ...
              <button style={{ ...btn(scraping,C.blueL), background:scraping?"#93C5FD":C.blueL }} onClick={scrapeMarket} disabled={scraping}>
                {scraping?"⏳ Recherche en cours…":`🔍 Booking & Airbnb · ${selWeek?.label}`}
              </button>
