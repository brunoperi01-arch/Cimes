...
Ne jamais inventer un prix precis si aucun n'est fourni : mets detected_price a null. Score de comparabilite eleve si meme zone, meme capacite, meme type de bien que Les Cimes (residence avec services).`;
  ...
  const Diagnostic=()=> (
    <Admin rates={rates} settings={settings} imports={imports} reco={reco} user={user}
      selWeekId={selWeekId} cap={cap} ourRates={ourRates} currentOurRate={currentOurRate}
      ourRateSource={ourRateSource} showExamples={showExamples} sbErrors={sbErrors} SB_READY={SB_READY}
      fmt={fmt} daysSince={daysSince} styles={{ cnt, cd, sml, rw }} SBar={SBar} BNav={BNav} />
  );
