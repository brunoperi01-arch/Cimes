...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: '20px 0 40px' }}>
      <div style={SHELL}>
        {screen === 'dashboard' && (
          <DashboardPage {...shared} {...state} ratesMap={{}} />
        )}

        {screen === 'weeks' && (
          <WeeksPage {...shared} {...state} onSelectWeek={selectWeek} ratesMap={{}} />
        )}
        ...
        {screen === 'import' && (
          <ImportPage {...shared} {...state} onImportCsv={handleImportCsv} />
        )}

        {screen === 'diag' && (
          <DiagnosticPage {...shared} {...state} />
        )}
      </div>
    </div>
  )
